...
const FlashcardStudyIsNotMemeory = () => {
    ...
    const handleMemorization = async (memorized) => {
        try {
            const currentCard = cards[currentIndex];
            const endpoint = memorized ?
                `/flashcards/isMemoried/${currentCard.id}` :
                `/flashcards/isNotMemoried/${currentCard.id}`;
    ...
    if (!cards.length) return (
        <>
            ...
            <div>Chưa có thẻ nào được thêm vào</div>
        </>
    )
        ;
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
            ...
            {/* Add Memorization Controls */}
            <div className="flex justify-center mb-4 space-x-4">
                <button
                    onClick={() => handleMemorization(true)}
                    className={`px-4 py-2 rounded-lg flex items-center space-x-2 
                        ${cards[currentIndex]?.memorized ?
                ...
                <button
                    onClick={() => handleMemorization(false)}
                    className={`px-4 py-2 rounded-lg flex items-center space-x-2 
                        ${!cards[currentIndex]?.memorized ?
